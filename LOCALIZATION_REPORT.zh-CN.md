...
Qt 6.7.3 `lupdate` 已全量扫描 `src`，移除 8 条过期 source，并补齐 94 条新提取文案。`lrelease` 已生成 481 条完成翻译的 `zh_CN.qm`。最终 QM 由 CI 重新生成并嵌入 EXE，不要求用户安装外部语言文件。
